...
struct ContentView: View {
    ...
    var mainView: some View {
        Group {
            if folderViewModel.showAllNotes {
                ...
                NavigationSplitView(columnVisibility: $columnVisibility) {
                    ...
                    .onChange(of: folderViewModel.showAllNotes) { _, showAllNotes in
                        // 当切换到全部笔记视图时，清除文件夹选择
                        print("ContentView: showAllNotes 状态变化为: \(showAllNotes)")
                        print("ContentView: 当前columnVisibility: \(columnVisibility)")
                        if showAllNotes {
                            print("ContentView: 清除selectedFolder并设置为doubleColumn")
                            folderViewModel.selectedFolder = nil
                            // 在iPhone上需要使用detailOnly来显示中间列的内容
                            if horizontalSizeClass == .compact {
                                columnVisibility = .detailOnly
                                print("ContentView: iPhone模式，设置columnVisibility为detailOnly")
                            } else {
                                columnVisibility = .all
                                print("ContentView: iPad模式，设置columnVisibility为all")
                            }
                            print("ContentView: 最终columnVisibility为: \(columnVisibility)")
                        } else {
                            // 当从全部笔记视图返回时，确保正确的列可见性
                            print("ContentView: 从全部笔记视图返回")
                            if horizontalSizeClass == .compact {
                                columnVisibility = .doubleColumn
                                print("ContentView: iPhone模式，设置为doubleColumn")
                            } else {
                                columnVisibility = .all
                                print("ContentView: iPad模式，设置为all")
                            }
                        }
                    }
                    ...
                    // Third column: Note editor or All Notes view
                    Group {
